...
export async function getTaskHistory(
	offset: number, limit: number = 10, recordId?: string
): Promise<{ count: number, rows: Task[] } & SuccessResponse | ErrorResponse> {
	try {
		...
		return {
			...data,
			rows: data.rows.map((t: { intervals: { startAt: string, endAt: string | null }[] }) => ({
				...t,
